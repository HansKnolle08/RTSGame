...
@onready var rts_selection_manager := $RTSSelectionManager
...
## public methods
func camera_input_handler(camera:TYPE_RTS_CAMERA, delta: float):
...
func selection_dragbox() -> void:
	if Input.is_action_pressed(&"input_action_mouseclick_left"):
		...
		rts_selection_manager.update_selection_rectangle(Rect2(mouse_dragbox_start_position, mouse_dragbox_end_position - mouse_dragbox_start_position))
